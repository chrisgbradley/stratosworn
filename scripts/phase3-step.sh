...
stop_java () { powershell -NoProfile -Command "Get-Process java -ErrorAction SilentlyContinue | ForEach-Object { \$cl=(Get-CimInstance Win32_Process -Filter \"ProcessId=\$(\$_.Id)\").CommandLine; if (\$cl -like '*$1*') { Stop-Process -Id \$_.Id -Force -Confirm:\$false } }" ; }
start_server () { rm -f server/logs/latest.log; powershell -NoProfile -Command "Start-Process -FilePath java -ArgumentList '@user_jvm_args.txt','@libraries/net/neoforged/neoforge/21.1.250/win_args.txt','nogui' -WorkingDirectory '$ROOT/server' -WindowStyle Hidden -RedirectStandardOutput \"\$env:TEMP\stratos-server.out\" -RedirectStandardError \"\$env:TEMP\stratos-server.err\""; for i in $(seq 1 80); do grep -q "Done (" server/logs/latest.log 2>/dev/null && return 0; sleep 3; done; echo "server never reached Done"; return 1; }

if [ "$SIDE" != "client" ]; then
  echo "== server boot loop"
  python scripts/rcon.py stop >/dev/null 2>&1; sleep 5; stop_java win_args.txt; sleep 2
...
stop_java quickPlayMultiplayer; sleep 2
bash scripts/client-install.sh 2>&1 | grep -E "Downloaded|Failed|Finished"
rm -f telemetry/run/logs/latest.log
(cd telemetry && nohup ./gradlew runClientJoin --console=plain > "$TEMP/stratos-client.out" 2>&1 &)
for i in $(seq 1 120); do
  grep -q "joined the game" server/logs/latest.log 2>/dev/null && break
...
grep -q "joined the game" server/logs/latest.log || { echo "CLIENT never joined"; tail -5 telemetry/run/logs/latest.log | cut -c1-200; exit 3; }
